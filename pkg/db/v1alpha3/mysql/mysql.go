...
func (d *dbConn) RegisterExperiment(experiment *v1alpha3.Experiment) error {
...
func (d *dbConn) PreCheckRegisterExperiment(experiment *v1alpha3.Experiment) (bool, error) {
...
func (d *dbConn) GetExperiment(experimentName string) (*v1alpha3.Experiment, error) {
	var id string
	var paramSpecs string
	var objSpec string
	var algoSpec string
	...
	experiment := &v1alpha3.Experiment{
		Spec:   &v1alpha3.ExperimentSpec{},
		Status: &v1alpha3.ExperimentStatus{},
	}
	...
	if paramSpecs != "" {
		experiment.Spec.ParameterSpecs = new(v1alpha3.ExperimentSpec_ParameterSpecs)
	...
	if objSpec != "" {
		experiment.Spec.Objective = new(v1alpha3.ObjectiveSpec)
	...
	if algoSpec != "" {
		experiment.Spec.Algorithm = new(v1alpha3.AlgorithmSpec)
	...
	if nasConfig != "" {
		experiment.Spec.NasConfig = new(v1alpha3.NasConfig)
...
func (d *dbConn) GetExperimentList() ([]*v1alpha3.ExperimentSummary, error) {
	rows, err := d.db.Query(`SELECT name, status, start_time, completion_time FROM experiments`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var result []*v1alpha3.ExperimentSummary
	var start_time string
	var completion_time string
	for rows.Next() {
		experiment_sum := v1alpha3.ExperimentSummary{
			ExperimentName: "",
			Status:         &v1alpha3.ExperimentStatus{},
		}
		err = rows.Scan(
...
func (d *dbConn) UpdateExperimentStatus(experimentName string, newStatus *v1alpha3.ExperimentStatus) error {
	start_time := ""
	completion_time := ""
	var err error
...
func (d *dbConn) UpdateAlgorithmExtraSettings(experimentName string, extraAlgorithmSetting []*v1alpha3.AlgorithmSetting) error {
...
func (d *dbConn) GetAlgorithmExtraSettings(experimentName string) ([]*v1alpha3.AlgorithmSetting, error) {
	rows, err := d.db.Query("SELECT setting_name, value FROM extra_algorithm_settings WHERE experiment_name = ?", experimentName)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var result []*v1alpha3.AlgorithmSetting
	for rows.Next() {
		as := new(v1alpha3.AlgorithmSetting)
		err := rows.Scan(
			&as.Name,
			&as.Value,
		)
...
func (d *dbConn) RegisterTrial(trial *v1alpha3.Trial) error {
	var objSpec string
...
func (d *dbConn) GetTrialList(experimentName string, filter string) ([]*v1alpha3.Trial, error) {
	var id string
	var objSpec string
	var paramAssignment string
	var start_time string
	...
	defer rows.Close()
	var result []*v1alpha3.Trial
	for rows.Next() {
		trial := &v1alpha3.Trial{
			Spec:   &v1alpha3.TrialSpec{},
			Status: &v1alpha3.TrialStatus{},
		}
		err := rows.Scan(
			&id,
			&trial.Name,
		...
		if objSpec != "" {
			trial.Spec.Objective = new(v1alpha3.ObjectiveSpec)
		...
		if paramAssignment != "" {
			trial.Spec.ParameterAssignments = new(v1alpha3.TrialSpec_ParameterAssignments)
		...
		if observation != "" {
			trial.Status.Observation = new(v1alpha3.Observation)
...
func (d *dbConn) GetTrial(trialName string) (*v1alpha3.Trial, error) {
	var id string
	var objSpec string
	var paramAssignment string
	var start_time string
...
func (d *dbConn) UpdateTrialStatus(trialName string, newStatus *v1alpha3.TrialStatus) error {
	var observation string = ""
