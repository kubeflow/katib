...
type SuggestionLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha3.Suggestion, err error)
...
// List lists all Suggestions in the indexer.
func (s *suggestionLister) List(selector labels.Selector) (ret []*v1alpha3.Suggestion, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha3.Suggestion))
	})
	return ret, err
}
...
type SuggestionNamespaceLister interface {
	...
	Get(name string) (*v1alpha3.Suggestion, error)
...
// List lists all Suggestions in the indexer for a given namespace.
func (s suggestionNamespaceLister) List(selector labels.Selector) (ret []*v1alpha3.Suggestion, err error) {
...
// Get retrieves the Suggestion from the indexer for a given namespace and name.
func (s suggestionNamespaceLister) Get(name string) (*v1alpha3.Suggestion, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha3.Resource("suggestion"), name)
	}
	return obj.(*v1alpha3.Suggestion), nil
}
