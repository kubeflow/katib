...
type TrialLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha3.Trial, err error)
...
// List lists all Trials in the indexer.
func (s *trialLister) List(selector labels.Selector) (ret []*v1alpha3.Trial, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha3.Trial))
	})
	return ret, err
}
...
type TrialNamespaceLister interface {
	...
	Get(name string) (*v1alpha3.Trial, error)
...
// List lists all Trials in the indexer for a given namespace.
func (s trialNamespaceLister) List(selector labels.Selector) (ret []*v1alpha3.Trial, err error) {
...
// Get retrieves the Trial from the indexer for a given namespace and name.
func (s trialNamespaceLister) Get(name string) (*v1alpha3.Trial, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha3.Resource("trial"), name)
	}
	return obj.(*v1alpha3.Trial), nil
}
