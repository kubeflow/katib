...
/**
 * Create a d3 graph with Parallel Coordinates
 */
export function parcoords(config) {
  ...
  function compute_control_points(centroids) {
    ...
    cps.push([
      centroids[0][0] + a * 2 * (centroids[1][0] - centroids[0][0]),
      centroids[0][1],
    ]);
    for (var col = 1; col < cols - 1; ++col) {
      ...
      var diff = vectorSubtract(left, right);
      cps.push(vectorAdd(mid, vectorMultiply(diff, a)));
      cps.push(mid);
      cps.push(vectorSubtract(mid, vectorMultiply(diff, a)));
    }
    cps.push([
      centroids[cols - 1][0] +
        a * 2 * (centroids[cols - 2][0] - centroids[cols - 1][0]),
      centroids[cols - 1][1],
    ]);
  ...
  function single_curve(d, ctx) {
    ...
    for (var i = 1; i < cps.length; i += 3) {
      if (__.showControlPoints) {
        for (var j = 0; j < 3; j++) {
          ctx.fillRect(cps[i + j][0], cps[i + j][1], 2, 2);
        }
      }
      ctx.bezierCurveTo(
        cps[i][0],
