...
example of Katib. Then you can get the new `Experiment` as below.
Katib concepts will be introduced based on this example.
...
For each set of hyperparameters, Katib will internally generate a `Trial` CR
with the hyperparameters key-value pairs, job manifest string with parameters
instantiated and some other fields like below. The `Trial` CR is used for
internal logic control, and end user can even ignore it.
...
$ kubectl get trial random-example-n9c4j4cv -o yaml -n kubeflow
...
Katib will internally create a `Suggestion` CR for each `Experiment` CR. The
...
hyperparameter in `status.suggestions`. Same as `Trial`, the `Suggestion` CR
...
controller, `Trial` controller and `Suggestion` controller is working together
...
1. The `Trial` controller generates `Worker Job` based on the `trialSpec`
   manifest with the new hyperparamters set.
