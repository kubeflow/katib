...
3. Update `check-katib-ready.sh`(../test/scripts/v1alpha3/check-katib-ready.sh) script to modify `katib-config.yaml` with new test suggestion image name. For example (Replace `<name>` with new suggestion name):
