...
A standardized development process for Katib is proposed, in order to:

- provide a common structure for proposing changes to Katib
...
We are proposing usage of KEPs in Katib to be better aligne with the rest of Kubeflow, as well as the wider Kubernetes community.
...
for maintainers to deliberate. Contributors proposing new KEPs should create a copy of the [template directory](https://github.com/kubeflow/community/tree/master/proposals/NNNN-template) and proceed as per the instructions in the template.
...
KEPs are checked into the katib repo under the `docs/proposals` directory.
