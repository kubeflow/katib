  ...
  */
  ...
  var root_path = $('.body').context.body.dataset.root;
    for (let heading of selectedHeadings) {
      var html = new EJS({url: root_path +'/ejs/table-heading.ejs'}).render({"heading": heading, "rootPath": root_path});
      ...
      if (models[index].show) {
        ...
        for (let heading of selectedHeadings) {
          ...
          var cellHtml = new EJS({url: root_path + '/ejs/table-cell.ejs'})
            .render({"heading": heading, "value": value, "index": index, "rootPath": root_path });
          row.append(cellHtml);
        }
  ...
  function addFilter(key, val) {
    ...
    var filterDiv = $(new EJS({url: root_path + '/ejs/filter.ejs'}).render(obj));
  ...
  function addRange(key) {
    var rangeDiv = $(new EJS({url: root_path + '/ejs/range.ejs'}).render({key: key}));
