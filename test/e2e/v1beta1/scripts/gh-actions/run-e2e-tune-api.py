...
# Test for Experiment created with custom objective and non-distributed training.
def run_e2e_experiment_create_by_tune_with_custom_objective_non_distributed(
...
# Test for Experiment created with custom objective and distributed training.
def run_e2e_experiment_create_by_tune_with_custom_objective_distributed(
    katib_client: KatibClient,
    exp_name: str,
    exp_namespace: str,
):
...
# Test for Experiment created with external models and datasets and non-distributed training.
def run_e2e_experiment_create_by_tune_with_external_model_non_distributed(
    katib_client: KatibClient,
    exp_name: str,
    exp_namespace: str,
):
...
# Test for Experiment created with external models and datasets and non-distributed training.
def run_e2e_experiment_create_by_tune_with_external_model_distributed(
    katib_client: KatibClient,
    exp_name: str,
    exp_namespace: str,
):
    ...
    katib_client.tune(
        ...
        resources_per_trial=types.TrainerResources(
            num_workers=1,
            num_procs_per_worker=1,
            resources_per_worker={"cpu": "4", "memory": "10G",},
        ),
    )
    experiment = katib_client.wait_for_experiment_condition(
        exp_name, exp_namespace, timeout=EXPERIMENT_TIMEOUT
...
if __name__ == "__main__":
    ...
    try:
        run_e2e_experiment_create_by_tune_with_custom_objective_non_distributed(katib_client, f"{exp_name}-1", exp_namespace)
        logging.info("---------------------------------------------------------------")
    ...
    try:
        run_e2e_experiment_create_by_tune_with_custom_objective_distributed(katib_client, f"{exp_name}-2", exp_namespace)
        logging.info("---------------------------------------------------------------")
    ...
    try:
        run_e2e_experiment_create_by_tune_with_external_model_non_distributed(katib_client, f"{exp_name}-3", exp_namespace)
        logging.info("---------------------------------------------------------------")
        logging.info(f"E2E is succeeded for Experiment created by tune: {exp_namespace}/{f"{exp_name}-3"}")
    except Exception as e:
        logging.info("---------------------------------------------------------------")
        logging.info(f"E2E is failed for Experiment created by tune: {exp_namespace}/{f"{exp_name}-3"}")
        raise e
    finally:
        # Delete the Experiment.
        logging.info("---------------------------------------------------------------")
        logging.info("---------------------------------------------------------------")
        katib_client.delete_experiment(f"{exp_name}-3", exp_namespace)
    
    try:
        run_e2e_experiment_create_by_tune_with_custom_objective_distributed(katib_client, f"{exp_name}-4", exp_namespace)
        logging.info("---------------------------------------------------------------")
        logging.info(f"E2E is succeeded for Experiment created by tune: {exp_namespace}/{f"{exp_name}-4"}")
    except Exception as e:
        logging.info("---------------------------------------------------------------")
        logging.info(f"E2E is failed for Experiment created by tune: {exp_namespace}/{f"{exp_name}-4"}")
        raise e
    finally:
        # Delete the Experiment.
        logging.info("---------------------------------------------------------------")
        logging.info("---------------------------------------------------------------")
        katib_client.delete_experiment(f"{exp_name}-4", exp_namespace)
