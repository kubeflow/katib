...
cd ${GO_DIR}
cp cmd/db-manager/v1beta1/Dockerfile .
gcloud builds submit . --tag=${REGISTRY}/${REPO_NAME}/v1beta1/katib-db-manager:${VERSION} --project=${PROJECT}
gcloud container images add-tag --quiet ${REGISTRY}/${REPO_NAME}/v1beta1/katib-db-manager:${VERSION} ${REGISTRY}/${REPO_NAME}/v1beta1/katib-db-manager:latest --verbosity=info
