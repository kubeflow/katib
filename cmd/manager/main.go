...
func (s *server) Check(ctx context.Context, in *health_pb.HealthCheckRequest) (*health_pb.HealthCheckResponse, error) {
	...
	if in != nil && in.Service != "grpc.health.v1.Health" {
	...
	// Check if we can connect to vizier-db since otherwise manager could not serve most of its methods.
	dbIf, err := kdb.New()
	if err != nil {
		resp.Status = health_pb.HealthCheckResponse_NOT_SERVING
		return &resp, fmt.Errorf("Failed to open db connection: %v", err)
	}
	err = dbIf.SelectOne()
	if err != nil {
		resp.Status = health_pb.HealthCheckResponse_NOT_SERVING
