...
def load_and_preprocess_data(dataset_dir, transformer_type, tokenizer):
    ...
    try:
        eval_data = dataset["eval"] or dataset["test"]
    except Exception:
        eval_data = None
...
def train_model(model, transformer_type, train_data, eval_data, tokenizer, train_args):
    ...
    print(f"train_loss={train_results.training_loss}")
...
if __name__ == "__main__":
    ...
    reference_args = transformers.TrainingArguments(output_dir=train_args.output_dir)
    for key, val in train_args.to_dict().items():
        old_attr = getattr(reference_args, key, None)
